...
off_t PhxLoader::read_data_section(const std::string &path, uintptr_t gpu_ptr,
                                  off_t data_offset, size_t data_size) {
    // Verify gpu_ptr is registered
    auto it = reg_map_.find(gpu_ptr);
    if (it == reg_map_.end()) {
        throw std::runtime_error(
            "PhxLoader::read_data_section: gpu_ptr " +
    ...
    if (fd < 0) {
        throw std::system_error(errno, std::system_category(),
                                "PhxLoader::read_data_section: open(" + path +
                                    ")");
    }
    ...
    try {
        ...
        if (fstat(fd, &st) != 0) {
            throw std::system_error(errno, std::system_category(),
                                    "PhxLoader::read_data_section: fstat(" +
                                        path + ")");
        }
        ...
        if (aligned_end < data_end_in_file && file_size < data_end_in_file) {
            throw std::runtime_error(
                "PhxLoader::read_data_section: requested data extends beyond "
        ...
        if (result < 0) {
            throw std::runtime_error(
                "PhxLoader::read_data_section: phxfs_read failed with " +
        ...
        if (static_cast<size_t>(result) < static_cast<size_t>(data_end_in_file - aligned_offset)) {
            throw std::runtime_error(
                "PhxLoader::read_data_section: short read, read only " +
