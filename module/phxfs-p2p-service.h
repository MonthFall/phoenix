 ...
 *   as DMA bus addresses (valid under IOMMU=pt -- the deployment
 *   contract for bare-metal GDS).
 *
 * Lifecycle:
 *   - Caller must call phxfs_p2p_deregister(handle) when done.
 *   - If the NVIDIA driver forcefully reclaims the pages (GPU context
 ...
 *     is a safe no-op on the NVIDIA side (just frees heap).
 ...
 *
 * @gpu_vaddr:   GPU virtual address to pin (must be 64 KiB aligned)
 * @length:      size in bytes (must be multiple of 64 KiB)
 ...
 *         -ENODEV  if Phoenix's nvidia_p2p symbols not loaded
 *         -ENOMEM  if allocation failed
 *         other    propagated from nvidia_p2p_get_pages
 */
