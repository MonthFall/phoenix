...
/* Cap on in-flight *bytes* per ring, on top of the op-count QD, so a batch of
 * large (up to 1 GiB) slices cannot post ~1 TiB of logical I/O at once and
 * swamp the block layer / DMA / page cache (P2-4). At least one slice is always
 * admitted, so this never deadlocks even for a slice larger than the cap. */
#define PHXFS_URING_MAX_INFLIGHT_BYTES (512ULL * 1024 * 1024)  /* 512 MiB */
 ...
 * leaked and no unloaded code runs later (P2-5).
 */
namespace {
struct RingHolder {
...
/* Indices of slices needing resubmission after a short read/write (P0-3). */
 ...
 * simply spans multiple waves and is never truncated (P1-3). Slice length uses
...
static size_t build_slices(struct phxfs_io_op_req *reqs, int n,
    ...
    while (i < n && s < PHXFS_SLICE_POOL) {
        ...
        while (off < total && s < PHXFS_SLICE_POOL) {
            size_t rem = total - off;  /* subtraction: no overflow (P1-3) */
 ...
 * (P0-3). thread_ring() lazily recreates on the next call.
 */
 ...
 * slices in flight; reap completions in batches (P2-3). Handles (P0-3):
...
static int uring_submit_batch(struct phxfs_io_op_req *reqs, int n,
     ...
     * sync engine for this thread (P2-4).
     */
    if (!thread_ring())
     ...
     * (P1-3). Common case (n <= pool, each req <= chunk) is a single wave.
     */
    int    cur_req = 0;
    size_t cur_off = 0;
    while (cur_req < n) {
    ...
    /* P1-6: a CQE may report that this fd/filesystem does not support the
     * io_uring op (-EOPNOTSUPP) even though pread/pwrite works. account() only
     * records a negative errno when the request made ZERO progress (P1-5), so
     * retrying such a request whole via the always-available sync engine is
     * side-effect-safe (no partial write to redo). */
    for (int k = 0; k < n; k++)
